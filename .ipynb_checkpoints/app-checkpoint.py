...
def ps_ratio_analysis(ticker, plot_start_date):
    """Performs historical P/S analysis."""
    ...
    # Consistent colors: P/S line is blue
    ...
    ax_rev = ax_ps.twinx()
    # Consistent colors: Revenue bars are orange
    bars_rev = ax_rev.bar(revenue_plot.index, revenue_plot.values, width=5, alpha=0.3, color="tab:orange", label="Quarterly Revenue", zorder=2)
...
def adj_close_analysis(ticker, start_date):
    ...
    fundamentals_df = pd.DataFrame({"Price": info.get("regularMarketPrice"), "Dividend Yield (%)": info.get("dividendYield", 0) * 100, "Trailing P/E": info.get("trailingPE"), "Asset Type": info.get("quoteType")}.items(), columns=["Field", ticker]).set_index("Field")
    ...
    ax2 = ax1.twinx()
    bar = ax2.bar(dividends_to_plot.index, dividends_to_plot['Dividends'], color='green', alpha=0.4, label='Dividends', width=10, zorder=2)
...
with st.sidebar.expander("Dividend & Price Analysis", expanded=True):
    ...
    if st.button("Analyze Dividends & Price"):
        config["dividend_analysis"]["tickers"] = [t.strip().upper() for t in div_tickers_input.splitlines() if t.strip()]
        config["dividend_analysis"]["start_date"] = div_start_date_input
        save_config(config)
        
        all_fundamentals, dividend_figs = [], {}
        for ticker in config["dividend_analysis"]["tickers"]:
            try:
                funda_df, div_fig = adj_close_analysis(ticker, div_start_date_input)
                if funda_df is not None: all_fundamentals.append(funda_df)
                if div_fig is not None: dividend_figs[ticker] = div_fig
            except Exception as e:
                st.error(f"Error analyzing {ticker} dividends: {e}")
        st.session_state.div_results = {"fundamentals": all_fundamentals, "figs": dividend_figs}
        st.rerun()
    ...
    if st.button("Analyze Valuation Ratios"):
        ...
        for ticker in pe_tickers:
            try:
                ...
                pe_results[ticker] = f"Valuation analysis not available for {ticker}. Reason: {e}"
...
with tabs[0]:
    if st.session_state.div_results:
        ...
        if st.session_state.div_results["fundamentals"]:
            combined_df = pd.concat(st.session_state.div_results["fundamentals"], axis=1)
            numeric_rows = ["Price", "Dividend Yield (%)", "Trailing P/E", "Annual Return (%)", "Annual Return Adj (%)", "Annual Volatility (%)", "Sharpe Ratio", "Sharpe Ratio Adj"]
            rows_to_format = [row for row in numeric_rows if row in combined_df.index]
            st.dataframe(combined_df.style.format("{:.2f}", subset=pd.IndexSlice[rows_to_format, :], na_rep="N/A"))
        
        st.header("Dividend & Price Analysis")
        for fig in st.session_state.div_results["figs"].values():
            st.pyplot(fig)
            plt.close(fig)
    else:
